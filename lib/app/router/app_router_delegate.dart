    ...
    return Navigator(
      ...
      pages: [
        if (appModel.initializing)
          MaterialPage(key: ValueKey('AppInitialize'), child: AppInitialize()),
        if (!appModel.initializing)
          MaterialPage(key: ValueKey('AppHome'), child: AppHome()),
        if (appModel.pageName == 'About')
          MaterialPage(key: ValueKey('About'), child: About()),
        if (appModel.pageName == 'PostShow' && appModel.resourceId != null)
          MaterialPage(
            key: ValueKey('PostShow'),
            child: PostShow(appModel.resourceId!, post: postShowModel.post),
          ),
